...
from crud import (create_product, get_products, get_product, get_product_by_code, update_product, delete_product,
                  create_tax, get_taxes, generate_sales_report,create_transaction, create_transaction_detail)
...
def read_products(skip: int = 0, limit: int = 10, db = Depends(get_db)):
    return get_products(db, skip=skip, limit=limit)

@app.get("/products/{prd_id}", response_model=ProductSchema)
def read_product(prd_id: int, db = Depends(get_db)):
...
def read_product_by_code(code: str, db = Depends(get_db)):
    try:
...
def update_product_endpoint(prd_id: int, product: ProductCreate, db = Depends(get_db)):
...
def delete_product_endpoint(prd_id: int, db = Depends(get_db)):
    if not delete_product(db, prd_id=prd_id):
        raise HTTPException(status_code=404, detail="Product not found")
    return {"message": "Product deleted successfully"}

@app.post("/taxes/", response_model=TaxMasterSchema)
def create_tax_endpoint(entry: TaxMasterCreate, db = Depends(get_db)):
    return create_tax(db=db, entry=entry)

@app.get("/taxes/", response_model=list[TaxMasterSchema])
def read_taxes(skip: int = 0, limit: int = 10, db = Depends(get_db)):
    return get_taxes(db, skip=skip, limit=limit)

@app.post("/checkout")
def checkout(items: list[TransactionDetailCreate], db = Depends(get_db)):
    try:
